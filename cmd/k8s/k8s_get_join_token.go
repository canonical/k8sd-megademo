...
func newGetJoinTokenCmd(env cmdutil.ExecutionEnvironment) *cobra.Command {
	...
	cmd.Flags().BoolVar(&opts.worker, "worker", false, "generate a join token from a cluster control plane for a worker node. Specifying the node name of the worker node is optional")
